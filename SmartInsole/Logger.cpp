...
void SetupLogger(void) {
    Serial.println("SetupBL: Start");
    String DeviceName = "SmartInsole";
    DeviceName += (SideFlag == 0) ? "L" : "R";
    Serial.printf("SetupBL: DeviceName = %s\n", DeviceName.c_str());


    BLEDevice::init(DeviceName.c_str());
    Serial.println("SetupBL: BLEDevice init done");

    BLEDevice::setMTU(BLE_MTU_Size);
    Serial.println("SetupBL: setMTU done");

    pServer = BLEDevice::createServer();
    Serial.println("SetupBL: createServer done");

    // Accelerometer Service
    pAccelService = pServer->createService(
        SideFlag == 0 ? UUID_ACCEL_SERVICE_LEFT : UUID_ACCEL_SERVICE_RIGHT);
    Serial.println("SetupBL: createService done (pAccelService)");
    ...
    Serial.println("SetupBL: createCharacteristic done (pAccelCharacteristic)");

    pAccelCharacteristic->addDescriptor(new BLE2902());
    Serial.println("SetupBL: added BLE2902 descriptor");
    ...
    Serial.println("SetupBL: createService done (pPressureService)");
    ...
    pPressureCharacteristic = pPressureService->createCharacteristic(
        ...
        BLECharacteristic::PROPERTY_NOTIFY);
    Serial.println("SetupBL: createCharacteristic done (pPressureCharacteristic)");

    pPressureCharacteristic->addDescriptor(new BLE2902());
    Serial.println("SetupBL: added BLE2902 descriptor (pPressureCharacteristic)");
    

    // Start Services
    pAccelService->start();
    Serial.println("SetupBL: pAccelService->start() done");

    pPressureService->start();
    Serial.println("SetupBL: pPressureService->start() done");

    // Advertising
    BLEAdvertising *pAdvertising = BLEDevice::getAdvertising();
    Serial.println("SetupBL: gotAdvertising() done");

    pAdvertising->addServiceUUID(
        SideFlag == 0 ? UUID_ACCEL_SERVICE_LEFT : UUID_ACCEL_SERVICE_RIGHT);
    pAdvertising->addServiceUUID(
        SideFlag == 0 ? UUID_PRESSURE_SERVICE_LEFT : UUID_PRESSURE_SERVICE_RIGHT);
    Serial.println("SetupBL: added service UUIDs");

    pAdvertising->start();
    Serial.println("SetupBL: advertising started");
}
...
void testLogger() {
    ...
    Serial.println("Testing sendPressureArray...");
    sendPressureArray(testPressureArray, sizeof(testPressureArray) / sizeof(testPressureArray[0]), testTimestamp);
    Serial.println("sendPressureArray test completed!");

    // Test sendAccelerometerArray
    Serial.println("Testing sendAccelerometerArray...");
    ...
    Serial.println("sendAccelerometerArray test completed!");
